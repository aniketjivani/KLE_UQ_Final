...
os.makedirs(os.path.join(plot_dir, "rep_{:03d}".format(rep_num), "batch_{:02d}".format(batch_num)), exist_ok=True)
...
train_X = torch.tensor(hf_inputs, dtype=torch.double)
...
np.savetxt(os.path.join(input_dir,
                        "rep_{:03d}".format(rep_num),
                        "HF_Batch_{:03d}_Subset_Final.txt".format(batch_num + 1)), HFSubsetAugmented, fmt="%d")


# %%
# augment the training data with the new candidate values and save to respective text files.


# augmented_LF = np.zeros((mLF + 1, nLF))
#     augmented_LF[:mLF, :] = xiLF
#     augmented_LF[mLF, :] = tgEI[0]

#     HFSubsetAugmented = np.append(HFSubset, [mLF + 1])

#     # Save inputs to text files and acquired GP to pickle file. Use the newly generated text files to acquire fresh HF and LF simulation.

#     np.savetxt(os.path.join(args.dirInputs,
#                             "rep_{:03d}".format(args.repNum),
#                             "HF_Batch_{:03d}_Final.txt".format(batch_num + 1)), augmented_HF)
...
plt.savefig(os.path.join(plot_dir, 
            "rep_{:03d}".format(rep_num), 
            "batch_{:02d}".format(batch_num),"bo_contour_logEI_1d.png"), 
            bbox_inches='tight')
