...
if computeErrors == 1
    for repID in 1:args_dict["NREPS"]
        ...
        for batchID in 0:(N_ACQUIRED - 1)
            ...
            if batchID == 0
                ...
                HF_data = generateHF(x, xi_hf_scaled)
                LF_data = generateLF(x, xi_lf_scaled; chosen_lf=args_dict["chosen_lf"])
            ...
            if batchID == 0
                for case in kle_cases
                    ...
                    elseif case == "ra"
                        ...
                        QDelta_ra, λDelta_ra, bβDelta_ra, regDelta_ra, YMeanDelta_ra = buildKLE(xi_hf[(nHF + 1):end, :], Y_Delta[:, (nHF + 1):end], x; kle_kwargs_Δ...)
                    end
                end
            else
                for case in kle_cases
                    println("Rebuilding surrogate for case $case")
                    if case == "gp"
                        QLF_gp, λLF_gp, bβLF_gp, regLF_gp, YMeanLF_gp = buildKLE(xi_lf_scaled[1:nLF, :], LF_data[:, 1:nLF], x; kle_kwargs...)

                        QDelta_gp, λDelta_gp, bβDelta_gp, regDelta_gp, YMeanDelta_gp = buildKLE(xi_hf_scaled[1:nHF, :], Y_Delta[:, 1:nHF], x; kle_kwargs_Δ...)
                    elseif case == "ra"
                        QLF_ra, λLF_ra, bβLF_ra, regLF_ra, YMeanLF_ra = buildKLE(xi_lf_scaled[(nLF + 1):end, :], LF_data[:, (nLF + 1):end], x; kle_kwargs...)

                        QDelta_ra, λDelta_ra, bβDelta_ra, regDelta_ra, YMeanDelta_ra = buildKLE(xi_hf_scaled[(nHF + 1):end, :], Y_Delta[:, (nHF + 1):end], x; kle_kwargs_Δ...)
                    end
                end
            end
    ...
    if kle_kwargs_Δ.getAllModes == 1
        ...
        open(joinpath("./1d_toy/", @sprintf("err_heatmaps/err_heatmap_all_reps_case_%03d_%s_all_modes.jls", chosen_case, acqFunc)), "w") do io
            serialize(io, (gp_on_gp, gp_on_ra, ra_on_ra, ra_on_gp))
        end
    elseif kle_kwargs_Δ.getAllModes == 0
        # save results for all reps using Serialization 
        open(joinpath("./1d_toy/", @sprintf("err_heatmaps/err_heatmap_all_reps_case_%03d_%s.jls", chosen_case, acqFunc)), "w") do io
            serialize(io, (gp_on_gp, gp_on_ra, ra_on_ra, ra_on_gp))
        end
    end
elseif computeErrors == 0
    if kle_kwargs_Δ.getAllModes == 1
        all_error_data = deserialize(joinpath("./1d_toy/", @sprintf("err_heatmaps/err_heatmap_all_reps_case_%03d_%s_all_modes.jls", chosen_case, acqFunc)))
        gp_on_gp, gp_on_ra, ra_on_ra, ra_on_gp = all_error_data[1], all_error_data[2], all_error_data[3], all_error_data[4]
        
        npzwrite(joinpath("./1d_toy/", @sprintf("err_heatmaps/err_heatmap_all_reps_case_%03d_%s_all_modes.npz", chosen_case, acqFunc)),
        Dict("gp_on_gp"=>gp_on_gp, "gp_on_ra"=>gp_on_ra, "ra_on_ra"=>ra_on_ra, "ra_on_gp"=>ra_on_gp))
    elseif kle_kwargs_Δ.getAllModes == 0
        all_error_data = deserialize(joinpath("./1d_toy/", @sprintf("err_heatmaps/err_heatmap_all_reps_case_%03d_%s.jls", chosen_case, acqFunc)))
        gp_on_gp, gp_on_ra, ra_on_ra, ra_on_gp = all_error_data[1], all_error_data[2], all_error_data[3], all_error_data[4]

        npzwrite(joinpath("./1d_toy/", @sprintf("err_heatmaps/err_heatmap_all_reps_case_%03d_%s.npz", chosen_case, acqFunc)),
        Dict("gp_on_gp"=>gp_on_gp, "gp_on_ra"=>gp_on_ra, "ra_on_ra"=>ra_on_ra, "ra_on_gp"=>ra_on_gp))
    end
end
